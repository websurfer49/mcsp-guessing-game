window.onload = function() {
  ...
  suite('Attaching event listeners', function () {

      // In each test you need a clean version of the DOM
      // so you need to add a new container DIV before each test,
      // and then add your DOM to that container
      //
      // This line just declares those variables in a way that tests can access them
      let button, placeholder, container
      ...
      suite("clicking on buttons", function () {
        ...
        test("it adds text to the button when it's clicked", function () {
          // this is the code that students have to write
          setupButtonClick()

          // Simulate clicking on the button
          button.dispatchEvent(new Event('click', {bubbles: true}))

          // check to make sure that the placeholder has the correct text
          assert.strictEqual(placeholder.innerText, "It worked!")
        })
      })

      suite("keyboard events", function () {

        test("it captures keydown events", function () {
          let downArrowEvent = new KeyboardEvent('keydown', {bubbles: true, key: 'ArrowDown'})
          document.dispatchEvent(downArrowEvent)

          assert.strictEqual(placeholder.innerText, 'down down')

          let upArrowEvent = new KeyboardEvent('keydown', {bubbles: true, key: 'ArrowUp'})
          document.dispatchEvent(upArrowEvent)

          assert.strictEqual(placeholder.innerText, 'up up')
        })
      })
